...
class OpportunityViewModel(val repository: CuriosityRepo = CuriosityRepo.getInstance()) :
    BaseViewModel() {

    val pagingConfig: PagingConfig = getDefaultPageConfig()
    fun fetchCuriosityImages(cam: String): Flow<PagingData<Photos>> {
        return repository.letCuriosityImagesFlow(pagingConfig = pagingConfig, cam, "Opportunity")
            .map { it.map { it} }
