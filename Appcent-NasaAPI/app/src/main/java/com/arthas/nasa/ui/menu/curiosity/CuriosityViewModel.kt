...
class CuriosityViewModel(val repository: CuriosityRepo = CuriosityRepo.getInstance()) :
    BaseViewModel() {

    val pagingConfig: PagingConfig = getDefaultPageConfig()
    fun fetchCuriosityImages(cam: String): Flow<PagingData<Photos>> {
        return repository.letCuriosityImagesFlow(pagingConfig = pagingConfig, cam, "Curiosity")
            .map { it.map { it } }
